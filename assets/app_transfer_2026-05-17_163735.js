...
function renderMaterialGrid(course) {
  ...
  if (state.currentSort === 'newest') materials.sort((a,b) => new Date(b.date) - new Date(a.date));
  else                                 materials.sort((a,b) => new Date(a.date) - new Date(b.date));
    ...
    grid.innerHTML = `
      <div style="grid-column:1/-1">
        <div class="empty-state">
          ...
          <div class="empty-desc">${search || state.currentFilter !== 'all'
            ? 'Coba kata kunci atau filter lain.'
            : `Upload foto catatan atau materi kuliah. Tersimpan otomatis ke <code>data/${esc(course.slug)}.json</code>`
          }</div>
          ${!search && state.currentFilter === 'all'
            ? `<button class="btn btn-primary" onclick="openAddMaterialModal('${course.id}')">Upload Materi Pertama</button>`
            : ''}
        </div>
      </div>
    `;
...
async function addMaterial() {
  ...
  try {
    ...
    formData.append('notes', document.getElementById('material-notes-input').value.trim());
